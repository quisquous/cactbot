...
export class CoinachReader {
  ...
  constructor(coinachPath: string | null, ffxivPath: string | null, verbose = false) {
    ...
    if (coinachPath === null || coinachPath.length === 0) {
    ...
    if (ffxivPath === null || ffxivPath.length === 0) {
...
export class CoinachWriter {
  ...
  constructor(cactbotPath: string | null, verbose: boolean) {
    ...
    if (cactbotPath === null || cactbotPath.length === 0)
...
const data${(type !== null && type.length > 0) ? ':' + type : ' '} = ${str}${
      asConst ? ' as const' : ''
    };
