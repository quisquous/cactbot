...
def translate_timeline(line, trans, not_used_trans):
    ...
    line = replace_text_re.sub(text, line, re.IGNORECASE)
...
def get_common_raidboss_replacements():
    ...
    original_array += [" 21:........:40000010:"]
    original_array += ["1B:........:"]
    original_array += ["Engage!"]
    original_array += ["Enrage"]
    original_array += ["\\.\\*.?is no longer sealed"]
    original_array += ["\\.\\*is no longer sealed"]
    original_array += ["\\.\\*.?will be sealed off"]
    original_array += ["\\.\\*will be sealed off"]
    common_replacements_found = False
    replacements_text_found = False
    common_replacements = ""
    try:
        with open(base_triggers_path() + "../common_replacement.js", "r", encoding="utf-8") as f:
            for line in f.readlines():
                if line[:-1] == "const commonReplacement = {":
                    common_replacements_found = True
                if line[:-1] == "  replaceText: {":
                    replacements_text_found = True
                elif common_replacements_found and line[:-1] == "};":
                    common_replacements = common_replacements[:-2].replace("[textKeys.Tank]", "\"[textKeys.Tank]\"").replace("[textKeys.Healer]", "\"[textKeys.Healer]\"").replace("[textKeys.DPS]", "\"[textKeys.DPS]\"")
                    common_replacements = common_replacements.replace("[textKeys.E]", "\"[textKeys.E]\"").replace("[textKeys.N]", "\"[textKeys.N]\"").replace("[textKeys.S]", "\"[textKeys.S]\"").replace("[textKeys.W]", "\"[textKeys.W]\"").replace("[textKeys.NE]", "\"[textKeys.NE]\"").replace("[textKeys.NW]", "\"[textKeys.NW]\"").replace("[textKeys.SE]", "\"[textKeys.SE]\"").replace("[textKeys.SW]", "\"[textKeys.SW]\"")
                ...
                elif common_replacements_found and replacements_text_found:
                    common_replacements += line[:-1]
            return [common_replacements] + original_array
    except Exception:
        traceback.print_exc()
        pass
...
def find_duplicate_translations(trans, locale):
    if trans.get(locale, None):
        for category in ["replaceSync", "replaceText"]:
            already_found = []
            remove = []
...
def check_not_used_trans_in_regex(not_used_trans, lines):
    for line in lines:
        if "regex:" in line or "netRegex:" in line:
            remove_trans = ""
            remove_in_category = []
            # find elemnts to remove them afterwards
            for category in not_used_trans:
                # only if category is replaceSync or replaceText
                if type(not_used_trans[category]) == dict:
                    # for every missing translation
                    for trans in not_used_trans[category]:
                        line = line.replace('\\', '')  # remove the slashes
                        trans = trans.replace('\\', '')  # remove the slashes
                        pattern = re.compile(trans, re.IGNORECASE)
                        search_result = pattern.search(line)
                        # only is trans is in line
                        if search_result:
                            # add category to array if not available already
            ...
            for category in remove_in_category:
                try:
                    ...
                    print(f"Key: {remove_trans} cannot be removed anymore!")
    return not_used_trans
    ...
    if args.timeline:
        for line in lines:
            ...
            if m:
                ...
                not_used_trans = check_not_used_trans_in_regex(not_used_trans, lines)  # we can use lines here as we return afterwards
...
if __name__ == "__main__":
    ...
    print(f"Start checking on: {args.file} with language {args.timeline}")
    main(args)
