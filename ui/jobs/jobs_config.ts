import { jobLocalizedFull } from '../../resources/party';
import UserConfig, { ConfigEntry } from '../../resources/user_config';
...
const localizeJob = (job: Job): LocaleText => {
  return {
    en: `Option ${jobLocalizedFull[job].en}`,
    cn: `模块：${jobLocalizedFull[job].cn ?? job}`,
  };
};
...
UserConfig.registerOptions('jobs', {
  options: [
    ...
    ...(jobs.map((job): ConfigEntry => {
      return {
        id: `Option${job}`,
        name: localizeJob(job),
        type: 'select',
        options: {
          en: {
            'Disabled': 'disabled',
