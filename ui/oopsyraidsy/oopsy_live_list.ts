...
export class DeathReportLive {
  ...
  private AppendDetails(
    ...
    if (amountClass !== undefined && amountClass.length > 0)
...
export class OopsyLiveList implements MistakeObserver {
  ...
  OnMistakeObj(timestamp: number, m: OopsyMistake): void {
    ...
    const blameText = (blame !== undefined && blame.length > 0)
  ...
  private MakeRow(): HTMLElement {
    ...
    div.addEventListener('click', () => {
      ...
      const str = (mistakeTime !== undefined && mistakeTime !== null && mistakeTime.length > 0)
        ? `[${mistakeTime}] ${mistakeText}`
        : mistakeText;
