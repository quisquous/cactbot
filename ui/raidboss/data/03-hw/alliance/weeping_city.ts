  ...
  triggers: [
    {
      ...
      netRegexDe: NetRegexes.message({ line: 'Noch 15 Sekunden, bis sich (?:(?:der|die|das) )?(?:Zugang zu(?:[rm]| den)? )?Spinnenfalle schließt.*?', capture: false }),
      netRegexFr: NetRegexes.message({ line: 'Fermeture d(?:e|u|es) (?:l\'|la |les? )?Domaine de la Tisseuse dans.*?', capture: false }),
      netRegexJa: NetRegexes.message({ line: '蜘蛛女の狩場の封鎖まであと.*?', capture: false }),
      netRegexCn: NetRegexes.message({ line: '距女王蛛猎场被封锁还有.*?', capture: false }),
      netRegexKo: NetRegexes.message({ line: '15초 후에 거미 여왕의 사냥터[이가] 봉쇄됩니다.*?', capture: false }),
    ...
    },
    {
      ...
      netRegexDe: NetRegexes.message({ line: 'Noch 15 Sekunden, bis sich (?:(?:der|die|das) )?(?:Zugang zu(?:[rm]| den)? )?Aureole schließt.*?', capture: false }),
      netRegexFr: NetRegexes.message({ line: 'Fermeture d(?:e|u|es) (?:l\'|la |les? )?Hauteurs de la pyramide dans.*?', capture: false }),
      netRegexJa: NetRegexes.message({ line: 'ピラミッド上部層の封鎖まであと.*?', capture: false }),
      netRegexCn: NetRegexes.message({ line: '距金字塔上层被封锁还有.*?', capture: false }),
      netRegexKo: NetRegexes.message({ line: '15초 후에 피라미드 상층부[이가] 봉쇄됩니다.*?', capture: false }),
      run: (data) => {
    ...
    },
    {
      ...
      netRegexDe: NetRegexes.message({ line: 'Noch 15 Sekunden, bis sich (?:(?:der|die|das) )?(?:Zugang zu(?:[rm]| den)? )?Kammer des Nullsteins schließt.*?', capture: false }),
      netRegexFr: NetRegexes.message({ line: 'Fermeture d(?:e|u|es) (?:l\'|la |les? )?Tombeau de la Clef de voûte dans.*?', capture: false }),
      netRegexJa: NetRegexes.message({ line: '要の玄室の封鎖まであと.*?', capture: false }),
      netRegexCn: NetRegexes.message({ line: '距契约石玄室被封锁还有.*?', capture: false }),
      netRegexKo: NetRegexes.message({ line: '15초 후에 쐐기 안치소[이가] 봉쇄됩니다.*?', capture: false }),
      run: (data) => {
