...
var FakeEncounter = function() {
  ...
  for (var i = 0; i < this.totalledFromCombatants.length; ++i) {
...
FakeEncounter.prototype.update = function(elapsedSeconds) {
  ...
  for (var i = 0; i < this.totalledFromCombatants.length; ++i) {
    var total = 0;
    var strKey = this.totalledFromCombatants[i];
...
FakeEncounter.prototype.getDpsUpdate = function() {
  ...
  this.combatants.sort(function(a, b) { return b.data.damage - a.data.damage; });
...
var FakeCombatant = function(name, job) {
...
FakeCombatant.prototype.update = function(elapsedSeconds) {
  ...
  var gcds = Math.floor((this.gcdRemainder + elapsedSeconds) / this.gcd);
  if (gcds == 0) {
  ...
  for (var i = 0; i < gcds; ++i) {
    ...
    if (Math.random() < this.missPercentagePerHit) {
      ...
      this.data.hits++;
      var damage = (0.9 + Math.random() * 0.2) * this.averageDpsPerHit;
